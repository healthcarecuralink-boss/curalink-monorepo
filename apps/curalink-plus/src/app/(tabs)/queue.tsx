...
export default function QueueScreen() {
  ...
  const {
    ...
    enabled: Boolean(userId) && tab === "active",
  });
  const {
    data: history,
  ...
  const listForTab = tab === "incoming" ? incoming : tab === "active" ? (activeJob ? [activeJob] : []) : history;
  const listError = tab === "incoming" ? incomingError : tab === "active" ? activeJobError : historyError;
  const refetchList = tab === "incoming" ? refetchIncoming : tab === "active" ? refetchActiveJob : refetchHistory;

  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Text style={styles.title}>Teleconsult queue</Text>
      ...
      {listError ? (
        ...
        <EmptyState icon={<Video size={26} color={accent} strokeWidth={1.6} />} title={`No ${tab} consults`} />
      ) : (
        <View style={{ gap: 10 }}>
          {listForTab.map((job) => (
            <Pressable key={job.id} style={styles.jobCard} onPress={() => router.push(`/consult/${job.id}`)}>
              <View style={{ flex: 1 }}>
